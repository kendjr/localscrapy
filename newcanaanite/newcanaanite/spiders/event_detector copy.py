...
class EventDetectorSpider(scrapy.Spider):
        ...
        with open(file_path, mode='r', newline='') as csvfile:
            ...
            for row in reader:
                if all(key in row for key in ['url', 'hub', 'name']):
                    ...
                    self.logger.warning(f"Skipping row due to missing data: {row}")
        return urls_info
    ...
    def parse(self, response):
        ...
        # Yield a placeholder or actual item for each URL to ensure the spider continues processing
        yield result  # This ensures the spider doesn't close prematurely

    def closed(self, reason):
        import json
        self.logger.info(f"Grouped results structure: {json.dumps(self.grouped_results, indent=2)}")
    ...
    #def closed(self, reason):
        ...
        #with open(os.path.join(os.path.dirname(__file__), '..', 'sources', 'sources.json'), 'w') as json_file:
        #    import json
        #    json.dump(self.grouped_results, json_file, indent=2, ensure_ascii=False)   
